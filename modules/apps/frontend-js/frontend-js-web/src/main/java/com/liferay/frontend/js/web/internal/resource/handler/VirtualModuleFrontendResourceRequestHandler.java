...
public class VirtualModuleFrontendResourceRequestHandler
	...
	public FrontendResource handleRequest(HttpServletRequest httpServletRequest)
		...
		String requestHash =
			requestURI.substring(openParethensisIndex+1, closeParethensisIndex);

		String servletContextName =
			requestURI.substring(0, openParethensisIndex);

		String servletContextHash =
			_hashedFilesRegistry.getServletContextHash(servletContextName);

		if (!Objects.equals(requestHash, servletContextHash)) {
			return null;
		}

		String resourceURI =
			portalContextPath + Portal.PATH_MODULE  + StringPool.SLASH +
			servletContextName + requestURI.substring(closeParethensisIndex + 1);
		...
		boolean immutable = HashedFilesUtil.getHash(requestURI) != null;
	...
	private static final Log _log = LogFactoryUtil.getLog(
		VirtualModuleFrontendResourceRequestHandler.class);
