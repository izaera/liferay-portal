...
public class ServePackageServlet extends HttpServlet {
	...
	protected void service(
		...
		if (Validator.isNull(identifier)) {
			response.sendError(
				HttpServletResponse.SC_NOT_FOUND, "Missing identifier");
		...
		if (pos >= 0) {
			path = identifier.substring(pos + 1);
			identifier = identifier.substring(0, pos);
		}

		if (Validator.isNull(path)) {
			response.sendError(
				HttpServletResponse.SC_NOT_FOUND, "No path to file specified");

			return;
		}

		PackageConfig pkgConfig = _packageRegistry.find(identifier);

		if (pkgConfig == null) {
			response.sendError(
				HttpServletResponse.SC_NOT_FOUND, "Unable to resolve package");
		...
		try (InputStream inputStream = url.openStream()) {
			...
			response.sendError(
				HttpServletResponse.SC_NOT_FOUND, "Unable to read file");

			_logger.log(Logger.LOG_ERROR, "Unable to read file", e);
